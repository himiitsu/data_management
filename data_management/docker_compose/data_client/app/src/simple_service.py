...
class Handler(http.server.SimpleHTTPRequestHandler):
    ...
    def get_user_watch_history(self) -> dict:
        user_id = self.path.split('/')[-1]
        logging.info(f'Поступил запрос по истории пользователя user_id={user_id}')
        redis_history = f'history:{user_id}'
        if redis_interactor.is_cached(redis_history):
            logging.info(f'История пользователя user_id={user_id} присутствует в кеше')
            response = redis_interactor.get_data(redis_history)
        else:
            logging.info(f'История пользователя user_id={user_id} отсутствует в кеше, выполняем запрос к Postgres')
            user_history = [None, None, None]
            try:
                ...
                logging.info(f'Произошла ошибка запроса к Postgres:\n{e}')
            response = [{'movie_id': m, 'rating': r, 'timestamp': datetime.utcfromtimestamp(t).strftime('%Y-%m-%d %H:%M:%S')} for m,r,t in user_history]
            logging.info(f'Сохраняем историю пользователя user_id={user_id} в Redis-кеш')
            redis_interactor.set_data(redis_history, response)
        return response
